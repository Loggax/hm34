...
function search() {
    ...
    for(let i = 0; i <= mustangs.length; i++) {
        // console.log(mustangs[i].model);
        
        if (inputValue == mustangs[i].model) {
            contentHolder.style.display = "none";
            console.log(inputValue);
            let contentBlock = `
                <div>
                    <ul>
                        <li>${mustangs[i].model}</li>
                        <li>${mustangs[i].year}</li>
                        <li>${mustangs[i].generation}</li>
                        <li>${mustangs[i].cilinders}</li>
                        
                    <ul>
                    <img src="${mustangs[i].image}" alt="mustang">
                </div>
            `;
            searchHolder.insertAdjacentHTML('beforeend', contentBlock);
        } 
